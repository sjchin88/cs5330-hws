...
def main(argv):
    ...
    util.show_example(leaf_train, labels_dict, grayscale=False)
    util.show_example(leaf_test, labels_dict, grayscale=False)
    ...
    result_save_path = dir_model + "/results_resnet_2/"
    if not os.path.exists(result_save_path):
        os.makedirs(result_save_path)
        # return None
    ...
    classifier = nn.Sequential(
        collections.OrderedDict(
            [
                ("fc", nn.Linear(network.fc.in_features, len(labels_dict))),
