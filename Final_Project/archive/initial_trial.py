...
class MyNetwork(nn.Module):
    ...
    def __init__(self, square_size, num_filter_conv1=10, num_filter_conv2=20, num_hidden_node=50, num_output=10):
        """_summary_
...
def plot_performance(train_losses, train_counter, test_losses, test_counter, accuracy_scores):
    ...
    plt.show()
    plt.figure(2)
    nepochs = range(10, len(accuracy_scores)*10 + 1, 10)
...
def main(argv):
    ...
    # show_example(loader=leaf_train)
    # Initialize the network, optimizer, and arrays to store train_loss, train_counter
    # Check if path exist
    result_save_path = save_path + "/results/"
    if not os.path.exists(result_save_path):
    ...
    network = MyNetwork(44, num_filter_conv1=10,
                        num_filter_conv2=20, num_hidden_node=320, num_output=184)
    optimizer = optim.SGD(network.parameters(), lr=learning_rate,
                          momentum=momentum)
    ...
    train_counter = []
    test_losses = []
    test_counter = [i*len(leaf_train.dataset)
                    for i in range(0, n_epoch + 1, 10)]
    accuracy_score = []
    # accuracy_counter = [i for i in range(0, n_epoch+1, 10)]

    test_network(network, leaf_train, test_losses)
    for epoch in range(1, n_epoch + 1):
        # Train the network
        train_network(network, optimizer, leaf_train, epoch,
                      log_interval, train_losses, train_counter, batch_size_train)

        # Test network every 10 epoch
        if epoch % 10 == 0:
            accuracy = test_network(network, leaf_train, test_losses)
            accuracy_score.append(accuracy)
            print(
