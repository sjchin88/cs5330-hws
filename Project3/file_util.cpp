...
/*
  ...
  The function returns a non-zero value in case of an error.
 */
int append_image_data_csv(char *filename, char *image_filename, std::vector<int> &image_data, int reset_file)
{
  char buffer[256];
  char mode[8];
  FILE *fp;
...
/*
  ...
  The function returns a non-zero value if something goes wrong.
 */
int read_image_data_csv(char *filename, std::vector<char *> &filenames, std::vector<std::vector<int>> &data, int echo_file)
{
  FILE *fp;
  int fval;
  char img_file[256];
...
// Custom method to extract the object name
string extractObjName(string str)
{
...
int readImgFiles(char *dirname, vector<string> &imgLists, vector<string> &objNames)
{
  ...
  while ((dp = readdir(dirp)) != NULL)
  {
        ...
        strstr(dp->d_name, ".tif"))
    {
      ...
      string tempN(dp->d_name);
      objNames.push_back(extractObjName(tempN));
